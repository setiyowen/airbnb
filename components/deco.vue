<template>
  <div class="mt-40 mx-auto">
    <h1 class="font-semibold  text-xl"> Découvrez les aventures Airbnb</h1>
    <p>Voyages de plusieurs jours organisés par des experts locaux avec activités, repas et logements compris</p>
  </div>
  <div class="flex items-center py-[40px] px-[10px] mx-auto">
    <div class="flex-1 ">
      <img src="/img/deco1.png" class="h-auto" />
      <p class="text-xs font-sm mb-1">Cayman Islands</p>
      <p class="text-base font-semibold mb-2">
        2 Nights PACKAGE All Inclusive
      </p>
      <p class="text-xs font-serif"> À partir de 577€/personne - 3 jours </p>
    </div>
    <div class="flex-1 ">
      <img src="/img/deco2.png" class="h-auto" />
      <p class="text-xs font-sm mb-1">Uruguay</p>
      <p class="text-base font-semibold mb-2"> 2 Nights PACKAGE All Inclusive </p>
      <p class="text-xs font-serif"> À partir de 577€/personne - 3 jours </p>
    </div>
    <div class="flex-1 ">
      <img src="/img/deco3.png" class="h-auto" />
      <p class="text-xs font-sm mb-1">Iceland</p>
      <p class="text-base font-semibold mb-2">
        2 Nights PACKAGE All Inclusive
      </p>
      <p class="text-xs font-base "> À partir de 577€/personne - 3 jours </p>
    </div>
    <div class="flex-1 ">
      <img src="/img/deco4.png" class="h-auto" />
      <p class="text-xs font-sm mb-1">Mongolia</p>
      <p class="text-base font-semibold mb-2">
        2 Nights PACKAGE All Inclusive
      </p>
      <p class="text-xs font-serif"> À partir de 577€/personne - 3 jours </p>
    </div>
    <div class="flex-1 ">
      <img src="/img/deco5.png" class="h-auto" />
      <p class="text-xs font-sm mb-1">Bahrain</p>
      <p class="text-base font-semibold mb-2">2 Nights PACKAGE All Inclusive</p>
      <p class="text-xs font-serif text-slate-800">
        À partir de 577€/personne - 3 jours
      </p>
    </div>
    <div class="flex-1 ">
      <img src="/img/deco6.png" class="" />
      <p class="text-xs font-sm mb-1">Bosnia and Herzegovina</p>
      <p class="text-base font-semibold text-ellipsis mb-2">2 Nights PACKAGE All Inclusive</p>
      <p class="text-xs font-serif">À partir de 577€/personne - 3 jours</p>
    </div>
  </div>
</template>
<loge />
<script>
export default {};
</script>

<style></style>
